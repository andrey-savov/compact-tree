...
- `vocabulary_size` keyword argument to `CompactTree.from_dict()` — controls the `lru_cache` capacity on each `MarisaTrie`. Pass `0` to disable the LRU cache entirely (now the default), `None` to auto-size to the full vocabulary (zero evictions), or a positive int to cap memory use at the cost of occasional re-traversals.
