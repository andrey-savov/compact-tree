...
def profile_lookup(tree: CompactTree, d: dict,
    ...
    for sort_key, title, n in [
        ("cumulative", "CUMULATIVE TIME (top 20)", 20),
        ("tottime",    "TOTAL (self) TIME (top 20)", 20),
        ("calls",      "MOST-CALLED FUNCTIONS (top 15)", 15),
    ]:
        print(f"\n{'='*80}")
        print(f" {title}")
        print(f"{'='*80}")
...
if __name__ == "__main__":
    ...
    if args.mode in ("build", "both"):
        ...
        # Build silently for lookup-only mode
